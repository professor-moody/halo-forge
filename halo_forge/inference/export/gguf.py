...
class GGUFExporter(ModelExporter):
    ...
    def __init__(self, config: Optional[ExportConfig] = None, llama_cpp_path: Optional[str] = None):
        """
        ...
        self.last_export_evidence: Dict[str, Any] = {}
    ...
    def export(
        ...
        **kwargs,
        ...
        self.last_export_evidence = {
            "requested_backend_quantization": quantization,
            "actual_backend_quantization": None,
            "unquantized_fallback_used": False,
        }
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if not success:
                success = self._convert_via_transformers(hf_path, output_path, quantization)
    ...
    def _convert_via_script(
        ...
        try:
            ...
            base_outtype = {
                ...
                "F32": "f32",
                "BF16": "bf16",
            }.get(quantization, "f16")
            intermediate_path = output_path.with_suffix(f".{base_outtype}.gguf")
            ...
            if quantization not in ("F16", "F32", "BF16"):
                ...
                if self._quantize_script and self._quantize_script.exists():
                    ...
                    self.last_export_evidence.update(
                        actual_backend_quantization=quantization,
                        unquantized_fallback_used=False,
                    )
                else:
                    ...
                    self.last_export_evidence.update(
                        ...
                        fallback_reason="llama-quantize was unavailable",
                    )
    ...
    def _convert_via_transformers(
        self, hf_path: Path, output_path: Path, quantization: str
    ) -> bool:
