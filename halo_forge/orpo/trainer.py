"""PyTorch ORPO trainer.

Wraps `trl.ORPOTrainer` so the loss math (NLL + log-odds) and TRL's
tested gradient handling stay upstream. Halo-forge owns dispatch,
run-id minting, output_dir layout, training_summary contract, and
recovery guidance — same surface area as SFT/DPO so the public API +
frontend handle ORPO runs without modification.

Output shape: `<output_dir>/training_summary.json` with `modality="orpo"`.
"""
...
from trl import ORPOTrainer as _TRLORPOTrainer
from trl import ORPOConfig as _TRLORPOConfig
...
class ORPOTrainer:
    """Halo-forge ORPO trainer (PyTorch path).

    Example:
        config = ORPOConfig(model_name="Qwen/Qwen2.5-3B-Instruct", beta=0.1)
    ...
    def train(
        ...
        print("=" * 70)
        print(f"ORPO TRAINING (beta={cfg.beta})")
        print("=" * 70)
    ...
    def _build_summary(
        ...
        *,
        ...
        # ORPO emits chosen/rejected reward + log-odds-ratio in log_history;
        # mirror the DPO surface so the frontend chart code can be one path.
        reward_acc = [
        ...
        log_odds = [
            float(entry["log_odds_ratio"])
            for entry in log_history
            if isinstance(entry, dict)
            and isinstance(entry.get("log_odds_ratio"), (int, float))
        ]
        ...
        cycle_summary = build_cycle_summary(
            ...
            extra={
                ...
                "log_odds_final": log_odds[-1] if log_odds else None,
                "beta": cfg.beta,
            },
        )
