    ...
    async def _launch_modality_train(self, modality: str):
        ...
        try:
            ...
            job_id = await self.training_service.launch_modality_train(
                ...
                source_ui_page="/training",
                cycles=data.cycles,
                learning_rate=data.learning_rate if modality in {"audio", "reasoning", "agentic"} else None,
                lr_decay=data.lr_decay,
                samples_per_prompt=data.samples_per_prompt if modality in {"vlm", "audio"} else None,
                temperature=data.temperature,
                keep_percent=data.keep_percent,
                reward_threshold=data.reward_threshold if modality in {"vlm", "audio"} else None,
                task=data.task if modality == "audio" else None,
                limit=data.limit if modality in {"vlm", "reasoning", "agentic"} else None,
                resume_from_cycle=data.resume_from_cycle,
                seed=data.seed,
                allow_prototype_train=data.allow_prototype_train,
            )
