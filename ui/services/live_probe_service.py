...
Tracked async execution for all-module live execution probe runs.
"""
...
DEFAULT_LIVE_PROBE_OUTPUT_ROOT = "results/readiness/live"
DEFAULT_CANONICAL_LIVE_REPORT_PATH = Path("results/readiness/all_module_live_execution.v1.json")


class LiveProbeService:
    """Launch and manage all-module live probe jobs as tracked runs."""
    ...
    def _build_command(
        ...
        *,
        ...
        script_path = Path.cwd() / "scripts" / "run_all_module_live_matrix.py"
        cmd = [
            sys.executable,
            str(script_path),
            "--live-profile",
    ...
    async def launch_live_probe(
        self,
        *,
        live_profile: str = "live-smoke-v1",
        output_root: str = DEFAULT_LIVE_PROBE_OUTPUT_ROOT,
        ...
        if profile not in {"live-smoke-v1", "live-local"}:
            raise ValueError("live_profile must be one of: live-smoke-v1, live-local")
        ...
        launch_args = self._sanitize_launch_args(
            {
                ...
                "report_file": report_file or "all_module_live_execution.v1.json",
                "strict": bool(strict),
        ...
        job = self.state.create_job(
            job_type="live_probe",
            name=f"All-Module Live Probe ({profile})",
        ...
        report_path = Path(str(report_file)).expanduser() if report_file else (job_output_dir / "all_module_live_execution.v1.json")
        ...
        try:
            ...
            print(f"[LiveProbeService] Failed to persist launch context: {e}")
        ...
        created_transition = {
            ...
            "to_status": "pending",
            "applied": True,
            "source": "live_probe_service.launch_live_probe",
            "reason": "job_created",
    ...
    async def relaunch_from_context(
        ...
        *,
        ...
        """Relaunch live probe command from persisted launch context."""
        context = read_launch_context(launch_context_file)
        if context.service != "live_probe":
            raise ValueError("launch context does not belong to live probe service")
        if context.job_type != "live_probe":
            raise ValueError("launch context job_type is not live_probe")

        args = dict(context.args)
        return await self.launch_live_probe(
            live_profile=str(args.get("live_profile") or "live-smoke-v1"),
            output_root=str(args.get("output_root") or DEFAULT_LIVE_PROBE_OUTPUT_ROOT),
    ...
    async def _launch_process(
        ...
        transitioned = self.state.update_job_status(
            job_id,
            "running",
            source="live_probe_service._launch_process",
    ...
    async def _stream_logs(self, job_id: str) -> None:
        ...
        stdout_log = output_dir / "stdout.log" if output_dir else None
        stdout_file = None
        if stdout_log:
        ...
        if job.stop_requested:
            transitioned = self.state.update_job_status(
                job_id,
                "stopped",
                source="live_probe_service._stream_logs",
    ...
    def _write_run_summary(self, job_id: str, return_code: int) -> None:
        ...
        report_path = Path(str(job.launch_args.get("report_file") or ""))
        ...
        payload = {
            ...
            "execution_mode": str(job.launch_args.get("live_profile") or "live-smoke-v1"),
            ...
            "artifact_pointers": {
                ...
                "stdout_log": str(stdout_log),
                "launch_context": str(launch_context),
                "live_report": str(report_path) if report_path else "",
            },
    ...
    async def stop_job(self, job_id: str, timeout: float = 30.0) -> bool:
        ...
        try:
            ...
            self.state.update_job_status(
                job_id,
                "stopped",
                source="live_probe_service.stop_job",
    ...
    def get_logs(self, job_id: str, last_n: Optional[int] = None) -> list[dict]:
        """Fetch buffered logs for a live probe job."""
...
_live_probe_service: Optional[LiveProbeService] = None


def get_live_probe_service(state=None) -> LiveProbeService:
    """Get or create live probe service singleton."""
    global _live_probe_service
    if _live_probe_service is None:
        if state is None:
            from ui.state import state as app_state

            state = app_state
        _live_probe_service = LiveProbeService(state)
    return _live_probe_service
