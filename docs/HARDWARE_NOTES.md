...
10. **MLX GRPO** supports reference-free and reference-model single-cycle GRPO on Apple Silicon: rollouts via `MLXRolloutGenerator`, scoring via the V1 verifier registry, group-relative advantages, and an eager policy update. Reference-model mode loads a frozen second copy and records response-level KL summary metadata. Multi-cycle GRPO remains a roadmap follow-up.
